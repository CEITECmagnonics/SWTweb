...
- **µBLS modeling** (`#/bls`): micro-focused Brillouin light scattering after Wojewoda et al., [PRB 110, 224428 (2024)](https://doi.org/10.1103/PhysRevB.110.224428) — thermal spectra (with parameter sweeps → 2D maps) computed with either the **Green-function formalism** (full air / cover / magnet / substrate optical stack + output analyzer) or the **reciprocity theorem** (~10× faster in-browser, 30× for sweeps; no dielectric stack or analyzer — peak positions match the Green-function result). (Coherent k-sensitivity was removed pending a correctness fix.)
